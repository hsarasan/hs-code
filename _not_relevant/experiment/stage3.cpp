...
bool check_for_ambiguity(const FileToCommits &file_to_commits) {
    for (const auto &entry : file_to_commits) {
        const auto &commits = entry.second;
        
        // Check if there are multiple commits with the same file_path and opaque_id
        if (commits.size() > 1) {
            for (size_t i = 0; i < commits.size(); i++) {
                for (size_t j = i + 1; j < commits.size(); j++) {
                    if (commits[i].first == commits[j].first && commits[i].second != commits[j].second) {
                        return true;  // Ambiguity found
                    }
                }
            }
        }
...
int main() {
    ...
    for (int i = 0; i < R; ++i) {
        ...
        if (check_for_ambiguity(file_to_commits)) {
